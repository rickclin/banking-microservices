...
class CardManagementHandler:
    ...
    def postTransaction(self, cardNumber, description, amount, entryMode):
      # 1. authorize payment
      clientPaymentAuth, transportPaymentAuth = self.createConnection('payment-authorization', PaymentAuthorization)
      ack = clientPaymentAuth.authorize(cardNumber, amount)
      transportPaymentAuth.close()
      if ack:
          # 2. insert transaction
          clientTranxHist, transportTranxHist = self.createConnection('transaction-history', TransactionHistory)
